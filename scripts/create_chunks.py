...
def chunk_documents_from_df(df_source: pd.DataFrame, chunk_size: int, chunk_overlap: int) -> pd.DataFrame:
    """
    ...
    for index, row in df_source.iterrows():
        ...
        # The original full text is no longer needed for the chunk entry; it will be replaced by the chunk text.
...
if __name__ == "__main__":
    ...
    if not INPUT_PATH:
        raise ValueError("PARQUET_PATH environment variable not set or is empty. Please check your .env file.")
    ...
    print(f"Starting memory-efficient chunking for {total_rows} documents in {num_batches} batches...")
