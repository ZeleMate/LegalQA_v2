...
def create_sample_parquet_incrementally(
    input_path: str, output_path: str, sample_fraction: float = 0.01
):
    """
    ...
    print(
        f"Created a final sample with {total_sampled_rows} rows "
        f"({sample_fraction * 100:.1f}% of original)."
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a memory-efficient sample from a Parquet file."
    )
    ...
    create_sample_parquet_incrementally(
        input_path=args.input_file, output_path=args.output_file, sample_fraction=args.fraction
    )
